...
use common\models\ar_inherit\Images;
...
class Storage
{
    ...
    public function generateFileName() {
        ...
        //M::printr($rand, '$rand');
        $fs_filename = strftime('%Y%m%d%H%M%S') . microtime(1) . $rand;
        return $fs_filename;
    }

    public function store($from = 'files') { //CMSH4Resource $resource, $dev_key = 'ecm:illustrations'
        ...
        try {
            if (1) {
                ...
                if ($from == 'files') {
                    $TMP = $_FILES[0];
                    //получить mimetype и расширение файла
                    $ext = '';
                    if ($TMP['type'] == 'image/jpg') $ext = 'jpg';
                    if ($TMP['type'] == 'image/jpeg') $ext = 'jpeg';
                    if ($TMP['type'] == 'image/png') $ext = 'png';
                    //сгенерировать уникальное имя файла
                    $oImages->fs_filename = $this->generateFileName();
                    $oImages->fs_filename .= ".{$ext}";
                    //M::printr($oImages->fs_filename, '$oImages->fs_filename');
                    $oImages->fs_saveto = "/images";
                    //M::printr($oImages->fs_saveto, '$oImages->fs_saveto');
                    $source = $TMP['tmp_name'];
                    //M::printr($source, '$source');
                    $dest = "{$storage}{$oImages->fs_saveto}/{$oImages->fs_filename}";
                    //M::printr($dest, '$dest');

                    //перенести файл из .../tmp в /storage/images
                    move_uploaded_file($source, $dest);

                    $oImages->title = $TMP['name'];
                    $oImages->meta_data = '';
                    $oImages->fs_saveto .= "/{$oImages->fs_filename}";
                    //$oImages->fs_alias = '';
                    $oImages->fs_md5hash = md5_file("{$storage}{$oImages->fs_saveto}");
                    $oImages->fs_filesize = 0;
                    $oImages->dt_created = new Expression('NOW()');
                    $oImages->dt_updated = new Expression('NOW()');
                    $oImages->is_trash = false;
                    $oImages->fs_extension = $ext;
                    $oImages->filename = $TMP['name'];
                    if (!$oImages->save()) {
                        throw new Exception('Can`t save Images $oImages');
                    }
                    $oImages = Images::find()->where(['id' => $oImages->id])->one();
                }
